...
class Encoder(nn.Module):
    def __init__(self, latent_dim=128, num_fourier_freqs=8,
        ...
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256, mlp=[256, 512], group_all=True)

        # 在几个关键位置插入 Dropout，抑制对训练点的记忆
        self.drop_sa1 = nn.Dropout(dropout)
        self.drop_sa2 = nn.Dropout(dropout)
        self.drop_bottleneck = nn.Dropout(dropout * 2)  # 瓶颈处更强
    ...
    def forward(self, xyz):
        ...
        l1_xyz = torch.cat([l1_xyz_u, l1_xyz_c, l1_xyz_i], dim=1)
        l1_points = torch.cat([l1_points_u, l1_points_c, l1_points_i], dim=1)
        l1_points = self.drop_sa1(l1_points)     #新增
...
class PointCloudVAE(nn.Module):
    ...
    def forward(self, xyz, aux_vec=None, query_points=None, aero_only=False):
        # --- Encoder 阶段 ---
        mu, logvar = self.encoder(xyz)

        # --- 潜在空间采样 ---
        z_geo = self.reparameterize(mu, logvar) if self.training else mu

        # --- 物理预测阶段 ---
        # 训练时用 z_geo（带采样噪声，天然正则化）；推理时用 mu（稳定）
        aero_pred = self.aero_decoder(mu)

        if aero_only:
            return None, mu, logvar, aero_pred
...
class PointNetPP_CL(nn.Module):
    def __init__(self,
        ...
        self.sa1 = PointNetSetAbstraction(
            npoint=1024, radius=0.2, nsample=64,  # 32→64
            in_channel=3, mlp=[64, 128, 256],group_all=False      # 加宽
        )
        # SA2: 同理
        self.sa2 = PointNetSetAbstraction(
            npoint=512, radius=0.4, nsample=128,   # 64→128
            in_channel=256, mlp=[256, 256, 512],group_all=False   # 加宽
        )
        # SA3
        self.sa3 = PointNetSetAbstraction(
            npoint=None, radius=None, nsample=None,
            in_channel=512, mlp=[512, 1024], group_all=True       # 512→1024
        )
        '''
        # 单路 PointNet++
        self.sa1 = PointNetSetAbstraction(
            npoint=1024, radius=0.2, nsample=32, in_channel=0,
            mlp=[64, 64, 128], group_all=False
        )
        self.sa2 = PointNetSetAbstraction(
            npoint=512, radius=0.4, nsample=64, in_channel=128,
            mlp=[128, 128, 256], group_all=False
        )
        self.sa3 = PointNetSetAbstraction(
            npoint=None, radius=None, nsample=None, in_channel=256,
    ...
    def forward(self, xyz, **kwargs):
        # xyz shape: [B, N, 6] (xyz + nxnynz)
        # 前3维是坐标，后3维是法向量特征
        xyz_coords = xyz[:, :, :3]       # [B, N, 3] 坐标
        xyz_normals = xyz[:, :, 3:]      # [B, N, 3] 法向量
        l1_xyz, l1_points = self.sa1(xyz_coords, xyz_normals)
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points)
        _, global_feat = self.sa3(l2_xyz, l2_points)
        x = global_feat.squeeze(1)
        return self.mlp(x)
...
class SDFDataset(Dataset):
    ...
    def _load_one(self, pair):
        with np.load(pair['pc_path']) as pc_raw:
            ...
            if has_pool:
                uniform_pool = pc_raw['uniform_pool'].astype(np.float32)
                curvature_pool = pc_raw['curvature_pool'].astype(np.float32)
                importance_pool = pc_raw['importance_pool'].astype(np.float32)
            else:
                # 兼容老数据：只有 4000 固定点
        ...
        return {
            ...
            'scale': float(normalization_scale) if np.ndim(normalization_scale) == 0
                     else float(np.asarray(normalization_scale).flatten()[0]),
        }
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            'scale': torch.from_numpy(np.asarray(scale_norm, dtype=np.float32).reshape(1)),
        }

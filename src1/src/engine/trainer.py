...
class AeroTrainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            if self.should_stop:
                if self.accelerator.is_main_process:
                    print(f"[EarlyStop] val_r2 连续 {self.early_stop_patience} 轮不下降，终止训练。")
                break
    ...
    def _train_one_epoch(self):
        ...
        for batch in self.dataloaders.train_loader:
            ...
            cl_pred_norm = self.model(points, aux_vec=None, query_points=None, aero_only=True)[-1]
            #cl_pred_norm = self.model(points)
            ...
            #loss = F.smooth_l1_loss(cl_pred_norm, cl_gt_norm)
            loss = F.huber_loss(cl_pred_norm, cl_gt_norm, delta=0.1)

            self.accelerator.backward(loss)
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)  #新增
            self.optimizer.step()
    ...
    def _evaluate_epoch(self, epoch, history_row, avg_train_loss, current_lr, train_time, should_save_ckpt):
        ...
        improved = val_metrics["r2"] > self.best_val_r2
        if improved:

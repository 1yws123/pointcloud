...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_Aerodecoder/checkpoints_stage2_测试900个数据_1(从头训2000epoch)')
    parser.add_argument('--epochs', type=int, default=2000)
    parser.add_argument('--batch_size', type=int, default=120)
    parser.add_argument('--lr', type=float, default=3e-4) # 推荐初始学习率
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:
            ...
            # ================== 🌟 极致数据增强区域 🌟 ==================
            b, n, c = pc.shape
            # 1. 块内乱序 (Shuffle within blocks) 彻底摧毁网络对特定点索引的记忆
            # 前4000, 中4000, 后4000 各自打乱
            u_idx = torch.randperm(4000, device=device)
            c_idx = torch.randperm(4000, device=device) + 4000
            i_idx = torch.randperm(4000, device=device) + 8000
            shuffled_idx = torch.cat([u_idx, c_idx, i_idx])
            pc = pc[:, shuffled_idx, :]

            # 2. 随机高斯噪声 (Jittering)，摧毁微小噪点指纹
            noise = torch.randn_like(pc) * 0.002
            pc = pc + noise

            # 3. 随机缩放 (Scale)，强化宏观形态学习
            scale = torch.empty(b, 1, 1, device=device).uniform_(0.98, 1.02)
            pc = pc * scale

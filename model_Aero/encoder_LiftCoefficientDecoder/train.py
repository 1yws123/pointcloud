...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_LiftCoefficientDecoder/checkpoints_1')
...
def validate(model, val_loader, device):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            cl_gt = batch['aero_label'][:, 0].to(device).float().view(-1, 1)
            
            # 我们只需要物理解码器的预测，query_points 可以传 None 节省显存
            _, _, _, cl_pred = model(points, query_points=None)
            
            mse = F.mse_loss(cl_pred, cl_gt)
            mae = F.l1_loss(cl_pred, cl_gt)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # 复合物理 Loss (MSE + MAE)
            loss_mse = F.mse_loss(cl_pred, cl_gt)
            loss_mae = F.l1_loss(cl_pred, cl_gt)
        ...
        print(f"E{epoch+1:04d} | Loss: {avg_train_loss:.6f} | Val MAE: {val_mae:.4f} | LR: {current_lr:.2e}")

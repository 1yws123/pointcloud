...
class LiftCoefficientDecoder(nn.Module):
    def __init__(self, latent_dim=128):
        ...
        self.self_attn = nn.MultiheadAttention(embed_dim=latent_dim, num_heads=8, batch_first=True, dropout=0.1)
    ...
    def forward(self, z):
        ...
        """
        # --- 分支 1: Self-Attention ---
        # 由于 z 是向量，我们需要扩充一个序列维度 (B, 1, 128) 以满足 Attention 的输入要求
        z_in = z.unsqueeze(1)
        z_attn, _ = self.self_attn(z_in, z_in, z_in)
        z_attn = z_attn.squeeze(1) # 变回 (B, 128)
        ...
        z_fused = ws[0] * z_attn + ws[1] * z_channel + ws[2] * z_mlp

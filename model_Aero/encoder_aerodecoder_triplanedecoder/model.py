...
def farthest_point_sample(xyz, npoint):
    ...
    if _HAS_POINTNET2_OPS and xyz.is_cuda:
        # pointnet2_ops 要求 contiguous float32, 返回 [B, npoint] int32
        return p2_utils.furthest_point_sample(xyz.contiguous(), npoint).long()
...
class Encoder(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # 🌟 专门为“边缘/锐利度”点云提取一个全局物理特征
        edge_max_pool = torch.max(l1_points_c, dim=1)[0] # [B, 128]
        edge_avg_pool = torch.mean(l1_points_c, dim=1)   # [B, 128]
        edge_feature = torch.cat([edge_max_pool, edge_avg_pool], dim=-1) # [B, 256] 专门代表后缘和小翼的特征
        ...
        max_pool = torch.max(l2_points, dim=1)[0]  # [B, 256]
        # Avg-Pooling 提取表面厚度、弯度和体积分布 (极其关键!)
        avg_pool = torch.mean(l2_points, dim=1)    # [B, 256]
        
        # 拼接成 512 维的纯正物理几何特征，专门送给气动分支
        phys_feature = torch.cat([max_pool, avg_pool], dim=-1) 
        final_aero_feature = torch.cat([phys_feature, edge_feature], dim=-1) # 变成 512 + 256 = 768 维
        ...
        return mu, logvar,x, final_aero_feature
...
class AeroDecoder(nn.Module):
    def __init__(self, latent_dim=768, output_dim=1):
        super(AeroDecoder, self).__init__()
        self.mlp = nn.Sequential(
            nn.Linear(latent_dim, 256),  # 第一步直接暴降到 256 维
            nn.LayerNorm(256),
            nn.GELU(),
            nn.Dropout(p=0.2),          # 🌟
            nn.Linear(256, 128),
...
class PointCloudVAE(nn.Module):
    def __init__(self, latent_dim, plane_resolution, plane_features, num_fourier_freqs=6, 
        ...
        self.aero_decoder = AeroDecoder(latent_dim=768,output_dim=1)  # 输出 CL 
    ...
    def forward(self, pc, query_points=None):
        # 1. 编码到隐空间
        mu, logvar, x, phys_feature= self.encoder(pc)
        z = self.reparameterize(mu, logvar)
        
        aero_pred = self.aero_decoder(phys_feature)    
        ...
        return sdf_pred, mu, logvar, aero_pred,x

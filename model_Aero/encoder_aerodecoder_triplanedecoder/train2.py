...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_aerodecoder_triplanedecoder/checkpoints_stage1+2从头训_2')
...
def main():
    ...
    optimizer = optim.AdamW([
        ...
        {'params': model_to_optim.aero_decoder.parameters(), 'lr': args.lr_aero, 'weight_decay': 1e-4} 
    ]) 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        w_aero = progress * 5      # 主气动 Loss
        w_reg = progress * 0.5        # 物理拓扑约束 Loss
        ...
        for data in train_loader:
            ...
            dist_feat = torch.cdist(phys_feat, phys_feat)
            dist_cl = torch.cdist(gt_cl_norm, gt_cl_norm)  # [B, B]
            # 防止对角线 0 和 极小方差导致除以 0
            dist_feat_norm = dist_feat / (dist_feat.mean() + 1e-8)
            dist_cl_norm = dist_cl / (dist_cl.mean() + 1e-8)

            loss_phys_reg = F.l1_loss(dist_feat_norm, dist_cl_norm)
        ...
        if val_r2 > best_r2 and epoch > 1: # 只有引入气动后才存模型
            best_r2 = val_r2

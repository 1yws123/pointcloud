...
# ===============================================================
# 新增：气动参数解码器 (LiftCoefficientDecoder)
# ===============================================================
class LiftCoefficientDecoder(nn.Module):
    def __init__(self, latent_dim=128, surface_dim=256):
        super(LiftCoefficientDecoder, self).__init__()
        
        # 1. Self-Attention Branch: 捕捉全局表面依赖关系
        # 使用 8 个头来增强表达能力
        # 加上 dropout=0.3 抑制过拟合
        self.self_attn = nn.MultiheadAttention(embed_dim=surface_dim, num_heads=8, batch_first=True, dropout=0.1)
        # 注意力出来后，映射回 128 维用于最终融合
        self.attn_proj = nn.Sequential(
            nn.Linear(surface_dim, latent_dim),
            nn.LayerNorm(latent_dim),
            nn.GELU()
        )

        # 2. SE Branch (Squeeze-and-Excitation): 重新加权特征通道
        # 经过一个 MLP (压缩再扩张) 然后经过 Sigmoid
        reduction = 4
        ...
        # 4. 可学习的融合权重 (w1, w2, w3)
        # 初始设为 1，模型会在训练中自动调整它们的重要性
        self.fusion_weights = nn.Parameter(torch.ones(3))
        
        # 5. Drag/Lift Decoder (Dd/Cl): 最后的三个 MLP 层
    ...
    def forward(self, z, surface_features):
        """
        z: (Batch_Size, 128) - 来自 PointNet++ Encoder 的潜码
        surface_features: (Batch_Size, 512, 256) - 表面特征
        """
        # --- 分支 1: 真正的表面 Self-Attention ---
        # 512 个表面点之间进行相互作用，捕捉诸如“机翼前缘如何影响后缘流场”的物理依赖！
        z_attn, _ = self.self_attn(surface_features, surface_features, surface_features)
        
        # 将 512 个处理后的点池化成一个全局向量，并降维到 128
        z_attn_pooled = torch.max(z_attn, dim=1)[0] # (B, 256)
        z_attn_fused = self.attn_proj(z_attn_pooled) # (B, 128)
        
        # --- 分支 2: SE Branch ---
        # 学习通道权重并作用于原始 z
        channel_weights = self.se_module(z)
        z_channel = z * channel_weights # 重新加权
        
        # --- 分支 3: MLP Branch ---
        z_mlp = self.local_branch(z)
        
        # --- 特征融合 (Weighted Fusion) ---
        # zfused = w1 * zattn + w2 * zchannel + w3 * zmlp
        # 使用 Softmax 归一化权重，使训练更稳定
        ws = F.softmax(self.fusion_weights, dim=0)
        z_fused = ws[0] * z_attn_fused + ws[1] * z_channel + ws[2] * z_mlp
        
        # --- 最终输出 ---
        # 经过 3 层 MLP 得到升力系数 Cl
        cl_pred = self.regression_head(z_fused)
...
class PointCloudVAE(nn.Module):
    def __init__(self, latent_dim, plane_resolution, plane_features, num_fourier_freqs=6, 
        ...
        self.cl_decoder = LiftCoefficientDecoder(latent_dim=latent_dim)  # 输出 CL 
        self.cd_decoder = LiftCoefficientDecoder(latent_dim=latent_dim)
    ...
    def forward(self, xyz, query_points=None):
        ...
        # 这样可以减少采样带来的随机扰动
        cl_pred = self.cl_decoder(mu, l2_points)
        cd_pred = self.cd_decoder(mu, l2_points)
        # --- 几何重构阶段 (Triplane) ---
        ...
        return sdf_pred, mu, logvar,cl_pred, cd_pred 

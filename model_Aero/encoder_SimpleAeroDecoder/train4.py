...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_SimpleAeroDecoder/checkpoints_4(测试20个数据)')
...
def main():
    ...
    for name, param in model.named_parameters():
        if "aero_decoder" in name or "encoder.sa3" in name or "encoder.fc" in name:
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # ======== 1. 输入端物理增强 (保护解冻的 sa3 和 fc 层不过拟合) ========
            B, N, _ = points.shape
            ...
            # B. 微小高斯噪声 (模拟真实世界的粗糙度，极强正则化)
            if model.training:
                jitter = torch.randn_like(points) * 0.001 
            ...
            _, mu, logvar, cl_pred_norm = model(points, query_points=None)
            
            # 2. 潜空间数据增强 (Latent Noise Injection) 
            # 这比坐标抖动更有效，直接强迫气动解码器对特征波动具有鲁棒性
            if model.training:
                # 注入 5% 的高斯噪声到均值向量中
                noise = torch.randn_like(mu) * 0.05 
                mu_noisy = mu + noise
            else:
                mu_noisy = mu
                
            # 3. 过 Aero Decoder
            cl_pred_norm = model.module.aero_decoder(mu_noisy)

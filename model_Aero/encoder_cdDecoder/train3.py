...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_cdDecoder/checkpoints_4')
...
def validate(model, val_loader, device, cd_mean, cd_std):
    """验证集评估"""
    model.eval()
    total_mse = 0
    total_mae = 0
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            cd_real_gt = batch['raw_cd'].to(device).float().view(-1, 1)
            
            # 我们只需要物理解码器的预测，query_points 可以传 None 节省显存
            _, _, _, cd_pred_norm = model(points, query_points=None)
            cd_pred_real = cd_pred_norm * cd_std + cd_mean
            mse = F.mse_loss(cd_pred_real, cd_real_gt)
            mae = F.l1_loss(cd_pred_real, cd_real_gt)
...
def main():
    ...
    cd_mean = full_dataset.cd_mean
    cd_std = full_dataset.cd_std
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # 复合物理 Loss (MSE + MAE)
            loss_mse = F.mse_loss(cd_pred_norm, cd_gt_norm)
            loss_mae = F.l1_loss(cd_pred_norm, cd_gt_norm)
        ...
        # 验证
        val_mse, val_mae = validate(model, val_loader, DEVICE, cd_mean, cd_std)
        ...
        if val_mae < best_mae:
            ...
            torch.save(model.module.state_dict() if is_parallel else model.state_dict(), 
                       os.path.join(args.save_dir, 'best_cd_model.pth'))
            print(f"  --> Best Model Saved Val Real MAE: {val_mae:.4f}.")

        if (epoch + 1) % 500 == 0:
            torch.save(model.module.state_dict() if is_parallel else model.state_dict(), 
                       os.path.join(args.save_dir, f'cd_model_epoch_{epoch+1}.pth'))

...
# 导入你的模块
from model3 import PointCloudVAE
...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/yuwenshi/B737/model_Aero/encoder_SimpleAeroDecoder/checkpoints_10(添加打乱)')
...
def validate(model, val_loader, device, cl_mean, cl_std, full_dataset):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            shift_norm = (shift - torch.tensor(full_dataset.shift_mean, device=device)) / (torch.tensor(full_dataset.shift_std, device=device) + 1e-8)
            scale_norm = (scale - full_dataset.scale_mean) / (full_dataset.scale_std + 1e-8)
            if not torch.is_tensor(scale_norm):
                scale_norm = torch.tensor(scale_norm, device=device)
            else:
                scale_norm = scale_norm.to(device)
            aux_vec = torch.cat([shift_norm, scale_norm], dim=1)    # [B, 4]

            # 我们只需要物理解码器的预测，query_points 可以传 None 节省显存
            _, _, _, cl_pred_norm = model(points, aux_vec=aux_vec, query_points=None)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            scale = batch['scale'].to(DEVICE)          # [B, 1]
            # --- 归一化 ---
            shift_norm = (shift - torch.tensor(full_dataset.shift_mean, device=DEVICE)) / (torch.tensor(full_dataset.shift_std, device=DEVICE) + 1e-8)
            scale_norm = (scale - full_dataset.scale_mean) / (full_dataset.scale_std + 1e-8)
            if not torch.is_tensor(scale_norm):
            ...
            # 1. 独立分段打乱 (保持特征被送入正确的 SA 层)
            num_u, num_c, num_i = 4000, 4000, 4000
            idx_u = torch.randperm(num_u, device=DEVICE)
            idx_c = torch.randperm(num_c, device=DEVICE) + num_u
            idx_i = torch.randperm(num_i, device=DEVICE) + num_u + num_c
            idx_shuffle = torch.cat([idx_u, idx_c, idx_i])
        ...
        # 验证
        val_mse, val_mae = validate(model, val_loader, DEVICE, cl_mean, cl_std, full_dataset)

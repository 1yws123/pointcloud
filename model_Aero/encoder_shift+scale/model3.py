...
class SimpleAeroDecoder(nn.Module):
    def __init__(self, latent_dim=128, aux_dim=4, output_dim=1):
        super(SimpleAeroDecoder, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(latent_dim+ aux_dim , 128),
            nn.LayerNorm(128),
            nn.ReLU(),
            #nn.Dropout(0.3),
    ...
    def forward(self, z, aux_vec):
        x = torch.cat([z, aux_vec], dim=1)
        return self.net(x)
...
class PointCloudVAE(nn.Module):
    def __init__(self, latent_dim, plane_resolution, plane_features, num_fourier_freqs=6, 
        ...
        self.aero_decoder = SimpleAeroDecoder(latent_dim=latent_dim, aux_dim=4, output_dim=1)  # 输出 CL
    ...
    def forward(self, xyz, aux_vec=None,query_points=None):
        # --- Encoder 阶段 ---
        # xyz: [B, N, 3]
        ...
        if aux_vec is not None:
            aero_pred = self.aero_decoder(mu, aux_vec)
        else:
            aero_pred = self.aero_decoder(mu, torch.zeros(mu.shape[0], 4, device=mu.device))

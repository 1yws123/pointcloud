...
class SDFDataset(Dataset):
    def __init__(self, pc_root_dir, aero_root_dir, sdf_dir, 
        ...
        self.cd_mean = np.mean(all_cds)
        self.cd_std = np.std(all_cds)
        print(f"📊 CL 统计: Mean={self.cl_mean:.4f}, Std={self.cl_std:.4f}")
        print(f"📊 CD 统计: Mean={self.cd_mean:.4f}, Std={self.cd_std:.4f}")
        ...
        for i, pair in enumerate(self.file_pairs):
            ...
            with np.load(pair['sdf_path']) as sdf_raw:
                #normalization_shift = sdf_raw['shifts'].astype(np.float32)
                #normalization_scale = sdf_raw['scale'].astype(np.float32)
            ...
            all_shifts.append(normalization_shift.reshape(1, 3))  # 保证shape
            all_scales.append(np.array(normalization_scale).reshape(1, 1))
            ...
            self.memory_cache.append({
                'pc_input': pc_input,           # Tensor
                'vol_points': vol_points,       # Tensor
                'vol_sdf': vol_sdf,             # Tensor
                'detail_points': detail_points, # Tensor
        ...
        # ===== 新增统计归一化参数 =====
        all_shifts = np.concatenate(all_shifts, axis=0)  # [样本数, 3]
        all_scales = np.concatenate(all_scales, axis=0)  # [样本数, 1]
        self.shift_mean = all_shifts.mean(axis=0)
        self.shift_std = all_shifts.std(axis=0)
        self.scale_mean = all_scales.mean()
        self.scale_std = all_scales.std()
        print(f"Shift mean: {self.shift_mean}, std: {self.shift_std}")
        print(f"Scale mean: {self.scale_mean}, std: {self.scale_std}")
    ...
    def __getitem__(self, index):
        ...
        # 1. 提取原始 shift 和 scale
        raw_shift = np.array(data['shift'], dtype=np.float32)
        raw_scale = np.array(data['scale'], dtype=np.float32)

        # 2. 对 shift 和 scale 进行 Z-score 标准化 (防止量级差异淹没特征)
        ...
        return {
            ...
            # === 新增（注意下方 memory_cache 需要先保存 shift/scale===）
            'shift': torch.from_numpy(shift_norm).view(3),
            'scale': torch.from_numpy(np.array(scale_norm, dtype=np.float32).reshape(1))
        }

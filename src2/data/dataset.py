...
class SDFDataset(Dataset):
    ...
    def __init__(
        ...
        jitter_std=1e-3,
        point_dropout=0.0,
        compute_stats=True,   # 新增：是否在 Dataset 内部计算均值方差
        cl_mean=None,
        cl_std=None,
        cd_mean=None,
        cd_std=None,
        ...
        # --- mean/std ---
        if cl_mean is not None and cl_std is not None and cd_mean is not None and cd_std is not None:
            self.cl_mean, self.cl_std = float(cl_mean), float(cl_std)
            self.cd_mean, self.cd_std = float(cd_mean), float(cd_std)
        elif compute_stats:
            print("🔍 预计算气动标签标准化参数...")
            all_cls, all_cds = [], []
            ...
            # 占位，后续由 bundle 统一设置
            self.cl_mean, self.cl_std = 0.0, 1.0
            self.cd_mean, self.cd_std = 0.0, 1.0
    ...
    def _load_one(self, pair):
        with np.load(pair['pc_path']) as pc_raw:
            ...
            rng = np.random.default_rng(seed=int(pair['file_id'].split('_')[1]))
            pc_u = self._sample_from_pool(uniform_pool, self.num_points_uniform, rng)
            pc_c = self._sample_from_pool(curvature_pool, self.num_points_curvature, rng)
            pc_i = self._sample_from_pool(importance_pool, self.num_points_importance, rng)
            pc_fixed = np.concatenate([pc_u, pc_c, pc_i], axis=0).astype(np.float32)

        # SDF 数据（aero_only 时跳过）
        if self.mode == 'aero':
    ...
    def __getitem__(self, index):
        ...
        cl_norm = (data['raw_cl'] - self.cl_mean) / (self.cl_std + 1e-8)
        cd_norm = (data['raw_cd'] - self.cd_mean) / (self.cd_std + 1e-8)
        ...
        return {
            ...
            'aero_label': torch.from_numpy(aero_label),
